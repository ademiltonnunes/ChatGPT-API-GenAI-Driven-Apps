...
def main(model, english, energy, pause, dynamic_energy, wake_word, verbose):
    ...
    threading.Thread(target=s.record_audio, args=(audio_queue, energy, pause, dynamic_energy)).start()
    threading.Thread(target=s.transcribe_forever, args=(audio_queue, result_queue, audio_model, english, wake_word, verbose)).start()
    threading.Thread(target=s.reply, args=(result_queue,verbose)).start()
...
class SpeechRecognation:
    ...
    def record_audio(self, audio_queue, energy, pause, dynamic_energy):
        r = sr.Recognizer()
    ...
    def transcribe_forever(self,audio_queue, result_queue, audio_model, english, wake_word, verbose):
        while True:
    ...
    def reply(self, result_queue, verbose):
        while True:
    ...
    def transcribe_audio(self, audio_path, model = "base"):    
        try:
            # Load model
            audio_model = whisper.load_model(model)

            # Transcribe audio
            result = audio_model.transcribe(audio_path, language='english')          
            ...
            e = ("Error transcribing audio:", str(e))
            return e
